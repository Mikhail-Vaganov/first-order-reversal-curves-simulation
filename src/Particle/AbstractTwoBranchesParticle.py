...
class AbstractTwoBranchesParticle(MagneticParticle):
    def __init__(self, path_to_data_file, bottom_to_upper_switching_field: float, upper_to_bottom_switching_field: float):
        super().__init__()
        ...
        a_upper_saturation = (self.upper_branch[len_upper_data-1,2]*self.upper_branch[len_upper_data-2,1] - self.upper_branch[len_upper_data-2,2]*self.upper_branch[len_upper_data-1,1])/(
        self.upper_branch[(len(self.upper_branch) - 12), 1] - self.upper_branch[(len(self.upper_branch) - 1), 1])

        b_upper_saturation = (self.upper_branch[(len(self.upper_branch) - 1), 2] - self.upper_branch[(len(self.upper_branch) - 2), 2]) / (
        self.upper_branch[(len(self.upper_branch) - 1), 1] - self.upper_branch[(len(self.upper_branch) - 2), 1])
    ...
    def _prepare_plot(self):
        ...
        h = np.concatenate((np.arange(0,1,0.01), np.arange(1, 0, -0.01), np.arange(0, -1, -0.01), np.arange(-1, 0.01, 0.01)))
        m = np.zeros([len(h),1])
    ...
    def apply_field(self, field_value: float):
        ...
        if self.branch==-1:
            if applied_field< self.min_data_field_bottom:
                self.magnetization = self.bottom_chi_zero * applied_field
            else:
                self.magnetization = self.interpolated_bottom_magnetization(applied_field)

        if self.branch==1:
            if applied_field>self.max_data_field_upper:
                self.magnetization = self.upper_saturation_magnetization(applied_field)
            else:
